...
- **Access key / secret key** — stored securely in the system keychain
...
Proxy settings are per-connection. The proxy password is stored separately in the system keychain.
...
- **S3-to-SFTP** and **SFTP-to-S3** cross-protocol transfers (via local temp directory)
- **Multipart upload** for large files with concurrent parts and automatic retries (threshold, part size, and concurrency are [configurable](preferences.md))
...
Press **Cmd+Y** to sync between panels. The sync dialog shows a diff view of all changes and lets you select which files to transfer.
