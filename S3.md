...
Furman can encrypt files on your machine before uploading to S3, using **AES-256-GCM** with an **Argon2id**-derived key. This ensures data is encrypted at rest even if you don't trust the storage provider.
...
| KDF | Argon2id (m=19 MiB, t=2, p=1) |
| Salt | 16 random bytes per file |
| Nonce | 12 random bytes per file |

Encryption metadata is stored as custom S3 object metadata and does not modify the object key or require any server-side support.
...
- **Server-Side Encryption** — view SSE algorithm, KMS key ID, bucket key status
