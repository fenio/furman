...
Furman auto-detects provider capabilities and adjusts the UI accordingly. Features like versioning, lifecycle rules, CORS, and Object Lock are conditionally shown based on what the provider supports. You can also override capabilities per-connection in the advanced settings.

| Provider | Status |
|----------|--------|
| AWS S3 | Full support (all features) |
| MinIO | Supported (most features, some API differences handled) |
| Backblaze B2 | Supported |
| Cloudflare R2 | Supported |
| DigitalOcean Spaces | Supported |
| Wasabi | Supported |
| Google Cloud Storage (S3-compat) | Supported |
| Any S3-compatible endpoint | Supported (with configurable capabilities) |
